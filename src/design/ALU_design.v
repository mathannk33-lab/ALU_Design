...
always @(posedge CLK or posedge RST) 
                ...
                if(MODE_NEW)
                    ...
                    begin
                        if(check)
                            begin
                                case(CMD_NEW)
                                    4'b0000:RES <= {{(WIDTH){1'b0}}, OPA_NEW & OPB_NEW};
                                    4'b0001:RES <= {{(WIDTH){1'b0}},~(OPA_NEW & OPB_NEW)};
                                    4'b0010:RES <= {{(WIDTH){1'b0}} ,OPA_NEW | OPB_NEW };
                                    4'b0011:RES <= {{(WIDTH){1'b0}}, ~(OPA_NEW | OPB_NEW)};
                                    4'b0100:RES <= {{(WIDTH){1'b0}}, OPA_NEW ^ OPB_NEW};
                                    4'b0101:RES <= {{(WIDTH){1'b0}}, ~(OPA_NEW ^ OPB_NEW)};
                                    4'b0110:RES <= {{(WIDTH){1'b0}}, ~OPA_NEW};
                                    4'b0111:RES <= {{(WIDTH){1'b0}}, ~OPB_NEW};
                                    4'b1000:RES <= {{(WIDTH){1'b0}}, OPA_NEW >> 1};
                                    4'b1001:RES <= {{(WIDTH){1'b0}}, OPA_NEW << 1};
                                    4'b1010:RES <= {{(WIDTH){1'b0}}, OPB_NEW >> 1};
                                    4'b1011:RES <= {{(WIDTH){1'b0}}, OPB_NEW << 1};
                                    4'b1100:
                                        begin
    ...
    always @(*)
        begin
			...
			temp=OPA_NEW+OPB_NEW+CIN;
                        carry=(OPA_NEW+OPB_NEW);
                        SIGOPA = OPA_NEW;
                        SIGOPB = OPB_NEW;
